...
export class Sound implements Playback {
  ...
  set onAudioSampleReceived(callback: TAudioSampleCallback) {
    if (global.__EXAV_setOnAudioSampleReceivedCallback == null) {
      if (Platform.OS === "android" || Platform.OS === "ios") {
        ...
        throw new Error(
          `'onAudioSampleReceived' is not supported on ${Platform.OS}!`
        );
      }
    }
    if (this._key == null) {
  ...
  static createAsync = async (
    ...
    const status: AVPlaybackStatus = await sound.loadAsync(
      source,
      initialStatus,
      downloadFirst
    );
  ...
  public static getAverageLoudness(
    sampleOrChannel: AudioSample | AudioChannel
  ): number {
  ...
  _callOnPlaybackStatusUpdateForNewStatus(status: AVPlaybackStatus) {
    const shouldDismissBasedOnCoalescing =
      ...
      Date.now() - this._lastStatusUpdateTime.getTime() <
        this._coalesceStatusUpdatesInMillis;

    if (
      this._onPlaybackStatusUpdate != null &&
      !shouldDismissBasedOnCoalescing
    ) {
  ...
  _subscribeToNativeEvents() {
    if (this._loaded) {
      this._subscriptions.push(
        ...
        this._eventEmitter.addListener(
          "didUpdateMetadata",
          this._internalMetadataUpdateCallback
        )
      );
